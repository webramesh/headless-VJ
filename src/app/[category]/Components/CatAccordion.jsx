    ...
    <div className="container mx-auto px-4 lg:px-0">
      {accordionItems.map((item) => (
        <div key={item.id} className="border-b mb-2 border-slate-200">
          ...
          >
            <div className="pb-5 bg-[#F5F5F5] flex flex-col sm:flex-row sm:justify-start sm:gap-8 lg:gap-24 text-xs sm:text-sm  pl-3 text-red-500">
              <div className="flex flex-col gap-2 mb-4 sm:mb-0">
                {accordionContent.slice(0, 5).map((content, index) => (
                  <div key={index} className="hover:text-red-700 cursor-pointer">
                    {content}
                  </div>
                ))}
              </div>
