...
const ImageCard = ({ imgSrc, category, title }) => {
    return (
      <div className="flex flex-col items-center">
        ...
        <div className="text-left w-full px-3 md:px-6"> {/* Added responsive padding for large screens */}
  ...
  const ImageCardRow = () => {
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mb-12"> {/* Adjusted grid for 4 columns on large screens */}
        {data.map((item, index) => (
          <ImageCard
            key={index}
            imgSrc={item.imgSrc}
